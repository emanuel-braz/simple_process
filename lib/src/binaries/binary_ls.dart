...
class BinaryLs {
  ...
  String at_ = '-@'; // Display extended attribute keys and sizes in long (-l) output.
  String one = '-1'; // (The numeric digit ``one''.)  Force output to be one entry per line.  This is the default when output is not to a terminal.
  String A = '-A'; // List all entries except for . and ...  Always set for the super-user.
  String a = '-a'; // Include directory entries whose names begin with a dot (.).
  String B = '-B'; // Force printing of non-printable characters (as defined by ctype(3) and current locale settings) in file names as \xxx, where xxx is the numeric value of the character in octal.
  String b = '-b'; // As -B, but use C escape codes whenever possible.
  String C = '-C'; // Force multi-column output; this is the default when output is to a terminal.
  String c = '-c'; // Use time when file status was last changed for sorting (-t) or long printing (-l).
  String d = '-d'; // Directories are listed as plain files (not searched recursively).
  String e = '-e'; // Print the Access Control List (ACL) associated with the file, if present, in long (-l) output.
  ...
  String G = '-G'; // Enable colorized output.  This option is equivalent to defining CLICOLOR in the environment.  (See below.)
  ...
  String i = '-i'; // For each file, print the file's file serial number (inode number).
  ...
  String l = '-l'; // The lowercase letter ``ell''.)  List in long format.  (See below.)  If the output is to a terminal, a total sum for
  String m = '-m'; // Stream output format; list files across the page, separated by commas.
  ...
  String p = '-p'; // Write a slash (`/') after each filename if that file is a directory.
  ...
  String T = '-T'; 
  String t = '-t'; // Sort by time modified (most recently modified first) before sorting the operands by lexicographical order.
  String u = '-u'; // Use time of last access, instead of last modification of the file for sorting (-t) or long printing (-l).
  String U = '-U'; // Use time of file creation, instead of last modification for sorting (-t) or long output (-l).
  String v = '-v'; // Force unedited printing of non-graphic characters; this is the default when output is not to a terminal.
  String W = '-W'; // Display whiteouts when scanning directories.  (-S) flag).
  String w = '-w'; // Force raw printing of non-printable characters.  This is the default when output is not to a terminal.
  String x = '-x'; // The same as -C, except that the multi-column output is produced with entries sorted across, rather than down, the col-
